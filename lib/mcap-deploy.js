...
var debug = require('debug')('mway:mcap-deploy');
...
function _request( params, customRequest ) {
    ...
    debug(JSON.stringify(params, null, ' '));

    var req = customRequest || request;
    req(params, function( err, response, body ) {
        if ( err ) {
            debug('_request error: %o', body);
            return deferred.reject(err);
        }
        var statusCode = parseInt(response.statusCode, 10);
        debug('_request response with statusCode %s and response %o', statusCode, body);

        if ( statusCode >= 400) {
            debug('_request reject');
...
var createZip = function (rootPath) {
    ...
    output.on('finish', function() {
      debug('Zip created at %s', zipFilePath);
    ...
    return _validateProject(rootPath)
      ...
      .then(function() {
        return deleteZip(options.file).then(function() {
          return options.baseurl + organization;
        });
      });
};
