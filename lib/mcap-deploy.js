...
var currentAuthentication = function (options, customRequest) {
    ...
    req.get(getEndpoint(options.baseurl, endpoint), {
        ...
        if (err) {
            deferred.reject(new VError(err, 'Connection to %s failed', httpResponse.request.href));
            return;
        }
        var data = null;
        try {
            ...
            deferred.reject(new VError(err, 'Server response from %s is corrupt', httpResponse.request.href));
            return err;
        }
...
var createZip = function (rootPath, dest) {
    ...
    mcapEasyZip(rootPath, zipFilePath, {
        pattern: getGlobPattern(rootPath)
    }, function (err, files) {
        if (err) {
            return deferred.reject(new VError(err, 'Zip creation failed %s', zipFilePath));
        }
